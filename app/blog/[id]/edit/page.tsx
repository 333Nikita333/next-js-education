import { getPostById } from "@/services/getPosts";
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";

type Props = {
  params: {
    id: string;
  };
};

async function updatePost(data: FormData) {
  //! обьявляем серверный экшен
  "use server";

  //! получаем данные из формы
  const { title, body, id } = Object.fromEntries(data);

  //! делаем PATCH запрос на изменение поста
  const response = await fetch(`http://localhost:3300/posts/${id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ title, body }),
  });

  const post = await response.json();

  //! ревалидируем страницу поста
  revalidatePath(`/blog/${post.id}`);
  //! перенаправляем пользователя на страницу с новым постом
  redirect(`/blog/${post.id}`);
}

export default async function Profile({ params: { id } }: Props) {
  const post = await getPostById(id);

  return (
    <div>
      <h1>Profile of {post.title}</h1>

      <form className="form" action={updatePost}>
        <input
          type="text"
          placeholder="title"
          required
          name="title"
          defaultValue={post.title}
        />
        <textarea
          placeholder="content"
          required
          name="body"
          defaultValue={post.body}
        />
        {/* это поле ввода используется для передачи идентификатора 
        поста в форму. Когда пользователь отправляет форму, 
        значение этого поля ввода будет доступно на сервере и 
        может быть использовано для обновления или сохранения 
        информации о посте. */}
        <input type="hidden" name="id" value={post.id} />
        <div>
          <input type="submit" value="Update post" />
        </div>
      </form>
    </div>
  );
}
